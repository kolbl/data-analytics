...
@pytest.mark.parametrize("func", [column_statistics, column_statistics_direct])
def test_public_and_module_import_same_result(func):
    df = pd.DataFrame({"a": [1.0, 2.0], "b": [3.0, 4.0]})
    pd.testing.assert_frame_equal(column_statistics(df), func(df))
...
def test_basic_numeric_stats():
    df = pd.DataFrame(
        {"a": [1.0, 2.0, 3.0], "b": [2.0, 4.0, 5.0], "c": [3.0, 6.0, 7.0]}
    )
    out = column_statistics(df)
...
def test_nan_counts_and_skipna_aggregates():
    ...
    assert out["nan_percentage"].tolist() == pytest.approx(
        [1 / 3, 1 / 3, 2 / 3]
    )
...
def test_numeric_only_ignores_non_numeric_columns():
    ...
    out = column_statistics(df, numeric_only=True)
...
def test_numeric_only_false_all_numeric():
    df = pd.DataFrame({"a": [1.0, 2.0], "b": [3.0, 4.0]})
    out_default = column_statistics(df)
    out_false = column_statistics(df, numeric_only=False)
...
def test_no_numeric_columns_raises():
    ...
    with pytest.raises(ValueError, match="No columns to analyze"):
        column_statistics(df, numeric_only=True)


def test_empty_columns_raises_when_numeric_only_false():
    df = pd.DataFrame()
    with pytest.raises(ValueError, match="no columns"):
        column_statistics(df, numeric_only=False)
...
def test_mode_clear_majority():
    df = pd.DataFrame(
        {"a": [1.0, 1.0, 2.0], "b": [1.0, 2.0, 2.0], "c": [1.0, 1.0, 1.0]}
    )
    out = column_statistics(df)
